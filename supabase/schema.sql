...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_whatsapp_greetings_at') THEN
    CREATE TRIGGER update_whatsapp_greetings_updated_at
      BEFORE UPDATE ON whatsapp_greetings
      FOR EACH ROW
      EXECUTE FUNCTION update_updated_at_column();
  END IF;
END $$;
